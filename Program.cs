...
while (!win)
{
    ConsoleKey touch = Console.ReadKey(true).Key;

    var nx2 = playerX;
    var ny2 = playerY;

    if      (touch == ConsoleKey.Z || touch == ConsoleKey.UpArrow)    ny2--;
    else if (touch == ConsoleKey.S || touch == ConsoleKey.DownArrow)  ny2++;
    else if (touch == ConsoleKey.Q || touch == ConsoleKey.LeftArrow)  nx2--;
    else if (touch == ConsoleKey.D || touch == ConsoleKey.RightArrow) nx2++;
    else if (touch == ConsoleKey.Escape) break;

    if (nx2 >= 0 && nx2 < width && ny2 >= 0 && ny2 < height && grid[nx2, ny2] != 1)
    {
        if (grid[nx2, ny2] == 3) win = true;
